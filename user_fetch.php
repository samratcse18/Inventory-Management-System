...
$query = "SELECT * FROM user_details WHERE user_type = 'user' ORDER BY user_id DESC ";
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo '<tr>
            ...
            <td>' . $row['user_name'] . '</td>
            <td>' . $status . '</td>
			<td><button type="button" name="update" id="'.$row["user_id"].'" class="btn btn-warning btn-xs update">Update</button></td>
			<td><button type="button" name="delete" id="'.$row["user_id"].'" class="btn btn-danger btn-xs delete" data-status="'.$row["user_status"].'">Delete</button></td>
        </tr>';
    }
} else {
    echo "0 results";
